...
## Bug #38: 豆包视频生成异步任务超时错误处理与用量扣费缺陷

**日期**：2026-07-11

### 问题描述
在使用 `/video/generations/async` 异步视频生成接口时，由于生成时间长导致后台轮询超时后，系统的任务状态（`task.status`）依然被错误地标记为 `"succeeded"`（成功），但结果中的 `media` 数组为空。由于状态被判定为成功，调用端的计费/用量系统会错误地执行扣费扣次数。

### 根本原因
在 `src/api/controllers/video.ts` 的 `createVideoCompletion` 中，如果视频轮询结果为空（超时），原逻辑仅仅在内容后面拼了 `(获取视频结果超时，请稍后在历史记录中查看)` 字样：
```typescript
} else {
    initialAnswer.choices[0].message.content += "\n\n(获取视频结果超时，请稍后在历史记录中查看)";
}
```
这导致接口没有抛出任何异常，而是以正常数据包返回。后台媒体任务管理器 `media-task-manager.ts` 的 `runTask` 拦截不到任何错误，从而将该任务更新为 `"succeeded"` 成功状态，进而导致客户端调用逻辑执行扣费扣次数。

### 修复方案
在 `createVideoCompletion` 的超时 `else` 分支中，直接抛出 `APIException`，确保轮询失败被 `mediaTaskManager` 捕获并将任务正确标记为 `"failed"`，返回超时提示。
```typescript
} else {
