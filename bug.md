...
## Bug #36: API 渠道账号繁忙状态（BUSY）死锁泄露问题 (BUSY Deadlock Leak)

**日期**：2026-06-19

### 问题描述
在高并发或长时间运行流式 API 响应时，若客户端提前断开连接（如中断生成、关闭标签页或取消请求），部分账号会永久卡在 `BUSY` 繁忙状态。卡在 `BUSY` 的账号将无法参与后续的轮询调度，最终导致所有账号全被锁死在繁忙状态，系统请求进入无限排队，报错 `暂无空闲账号，请求进入队列`。

### 根本原因
1. **客户端提前断开导致 Stream 挂起**：在流式响应分支中，当客户端断开连接时，Koa / Node.js 的底层 HTTP 链接关闭，但由于转换流的缓冲区没有完全排空，其无法正常触发底层 Stream 的 `end` 或 `close` 事件。导致我们在路由层监听 `s.on('end')` / `s.on('close')` 释放 Token 的回调永远不会触发。
2. **缺乏状态超时自动恢复**：`AccountManager` 中未针对 `BUSY` 状态设计 TTL，使得一旦有账号进入 `BUSY` 却未被显式释放，该状态就会在内存中永久驻留。

### 修复方案
1. **连接断开实时释放机制**：
   - 修改 `Request.ts` 包装类，引入 `this.ctx = ctx` 属性将 Koa Context 暴露给路由层。
   - 在 `chat.ts`、`images.ts`、`video.ts`、`music.ts` 路由的流式分支中，为底层 HTTP 连接挂载监听：`request.ctx.req.on('close', ...)`。
...
- **资源状态机必须有 TTL 保护**：凡是在内存中被标记为锁定状态（如 `BUSY`、`LOCKED` 等）的独占性共享资源，都必须在架构上添加一个防呆的超时保护检测，这是防止软件遭遇意外网络挂起或异常未捕获导致死锁的最基本且最有效的防御性编程实践。
