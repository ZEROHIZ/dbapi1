...
# [NEW] Browser-Related Server Memory Leak

## 1. 问题描述
当用户在管理后台点击“打开浏览器”后，服务器进程的内存占用会持续上涨，即使关闭浏览器后上涨也不会停止。涨的是 Node.js 服务器进程本身的内存，而不是浏览器进程�?
## 2. 根本原因分析
1.  **外部进程调用缺少超时控制**：`BrowserProfileManager` 中的 `getProcessesByUserDataDir`、`getProcessDetails` �?`killProcessTree` 等方法调�?`powershell.exe` �?`taskkill` 时没有设�?`timeout`。在某些 Windows 系统环境下，PowerShell 命令（尤其是 `Get-CimInstance`）可能会因为 WMI 故障或进程过多而挂起�?2.  **Watcher 叠加泄露**：系统每 5 秒通过 `setInterval` 轮询一次浏览器状态。如果某次轮询中�?PowerShell 调用挂起，对应的 async 函数就会一直处�?`pending` 状态留在内存中。由�?`setInterval` 不等待上一次完成，�?5 秒都会产生一个新的挂起函数及其实际占用的子进程句柄和缓冲区，导致内存持续上涨�?3.  **Promise 锁链条泄�?*：`runWithProfileLock` 使用�?Promise 链来保证操作原子性。如果某个操作（�?`warmupBrowser` 中的 Puppeteer 调用）挂起且没有超时，该 Profile 的锁将永远无法释放。后续的定时热机（Cron）会不断向该 Promise 链添加新�?`.then()` 回调，导致内存随时间缓慢增长�?
## 3. 修复方案
1.  **全面增加超时保护**：为所�?`execFile` 调用增加 `timeout: 15000` (15�? 的配置，确保外部命令不会永久阻塞 Node.js 事件循环�?2.  **�?setInterval 改为递归 setTimeout**：将 Session Watcher 改为在当前任务完成后再调度下一次任务，避免在任务执行缓慢或挂起时产生并发堆积�?3.  **增强 PowerShell 命令稳健�?*：在 PowerShell 脚本中增�?`-ErrorAction SilentlyContinue` 并优化查询逻辑�?4.  **优化锁机�?*：虽�?Promise 链是标准的，但应确保链中的每个环节（action）都有自己的超时控制，防止死锁�?
## 4. 经验与教�?- **所有的外部 I/O 必须有超�?*：无论是网络请求还是本地子进程调用，只要涉及跨进程通信，就必须假设对方可能会死锁或挂起�?- **避免并发轮询堆积**：对于耗时可能波动的定时任务，优先选择“任务完成后再设下一次定时”的模式，而不是固定频率的 `setInterval`�?- **监控 pending 任务**：在复杂的异步系统中，如果内存持续增长，应优先排查是否存在大量未完成�?Promise 或被持有的闭包�?
# #   [ 2 0 2 6 - 0 5 - 1 0 ]   OmȉhV�d\O�[�g�Rۏz�QX[�lo
 -   * * �sa�* * ��p�Q 
 Sb _OmȉhVT�g�Rۏz�QX[c�~�X��sQ�OmȉhVT�NN\Pbk0
 -   * * �S�VR�g* * �
         1 .   �p�Q�d\O��S�N  A c c o u n t M a n a g e r . u p d a t e A c c o u n t   �T  s a v e A c c o u n t s 0
         2 .   s a v e A c c o u n t s   �m�S�[@b	g&��S�S+T'Ypencϑ�v  C o o k i e / L o c a l S t o r a g e 	�ۏL�  J S O N   �^RS0
         3 .   ��A~�v�^RS�T  I O   �S���[�  V 8   �QX[�S�RǏ'Y0
         4 .   dkY��kR�� N!k�v  p r o b e B r o w s e r A c c o u n t s I f D u e   �[�e�N�R�S��(WSb _OmȉhVT�V�r`�SS��NuoR\O(u0
 -   * * Yt�eHh* * �͑�gSb _OmȉhV;����OvQb:N N*N�eoR\O(u�v�~�|/T�RhV0
  
 
---
...
### ��������
�첽�ӿڣ�ͼƬ/��Ƶ/���֣���ϵͳ���ľ�ʱ���Ի��ȳɹ��������� ID������ں�ִ̨��ʱ�ż�¼ʧ�ܡ��⵼�¿ͻ������õ� ID ��ʼ��ѯ��ֱ������״̬��Ϊ failed������ͻ��ˣ��� NewAPI���ڳ�ʼ���󱨴�ʱδ��ȷ���� ID�����ܻ���� prompt Ϊ�յ���Ч��ѯ��

### ����ԭ��
�������˺Ż�ȡ�߼�λ���첽�����ִ������executor���ڲ����������񴴽��׶Ρ����񴴽���ͬ����ɲ��������� ID �ġ�

### �޸�����
1. **ǰ�������**���� src/api/routes/media.ts �У��ڵ��� createTask ֮ǰ�Ƚ�����������֧����Ԥ��顣
2. **��ʱ����**��������������ֱ�ӷ��� 403 ���󣬲����������� ID��
3. **��ȷ������Ϣ**���Ż� sync-task-query �Ĵ�����ʾ��ָ���û�����ʼ�����Ƿ�ɹ���

### �������ѵ
- **����ʧ��ԭ��**��������֪��Ȼʧ�ܵ��첽����Ӧ���ύ�׶ξ����ز������������˷�ϵͳ��Դ�Ϳͻ�����ѯ����
- **�ӿ�³����**���������� ID �Ĳ�ѯ�ӿڣ����ṩ�����Ĳ���ȱʧ������ʾ��
