/**
 * @file common.ts
 * @description 即梦API通用常量定义，使用 Proxy 动态对接 JimengModelManager，实现模型映射的热更新。
 */
...
// 图像模型映射 (Proxy 实现，实时获取 JimengModelManager 中的映射配置)
export const IMAGE_MODEL_MAP = new Proxy({}, {
    get(target, prop) {
        const config = JimengModelManager.getModelConfig(String(prop));
        return config && config.enabled && config.type === "image" ? config.mappings.cn : undefined;
    }
}) as Record<string, string>;

export const IMAGE_MODEL_MAP_US = new Proxy({}, {
    get(target, prop) {
        const config = JimengModelManager.getModelConfig(String(prop));
        return config && config.enabled && config.type === "image" ? config.mappings.us : undefined;
    }
}) as Record<string, string>;

export const IMAGE_MODEL_MAP_ASIA = new Proxy({}, {
    get(target, prop) {
        const config = JimengModelManager.getModelConfig(String(prop));
        return config && config.enabled && config.type === "image" ? config.mappings.asia : undefined;
    }
}) as Record<string, string>;

// 视频模型映射 - 国内站 (CN)
export const VIDEO_MODEL_MAP = new Proxy({}, {
    get(target, prop) {
        const config = JimengModelManager.getModelConfig(String(prop));
        return config && config.enabled && config.type === "video" ? config.mappings.cn : undefined;
    }
}) as Record<string, string>;

// 视频模型映射 - 美国站 (US)
export const VIDEO_MODEL_MAP_US = new Proxy({}, {
    get(target, prop) {
        const config = JimengModelManager.getModelConfig(String(prop));
        return config && config.enabled && config.type === "video" ? config.mappings.us : undefined;
    }
}) as Record<string, string>;

// 视频模型映射 - 亚洲国际站 (HK/JP/SG)
export const VIDEO_MODEL_MAP_ASIA = new Proxy({}, {
    get(target, prop) {
        const config = JimengModelManager.getModelConfig(String(prop));
        return config && config.enabled && config.type === "video" ? config.mappings.asia : undefined;
    }
}) as Record<string, string>;
