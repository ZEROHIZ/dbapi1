/**
 * @file image-uploader.ts
 * @description 统一的图片上传模块，整合了图片Buffer与URL向即梦ImageX空间的上传逻辑。
 */

import crypto from "crypto";
import axios from "axios";
import { RegionInfo, request } from "@/jimeng/controllers/core.ts";
import { RegionUtils } from "@/jimeng/lib/region-utils.ts";
import { createSignature } from "@/jimeng/lib/aws-signature.ts";
...
export async function uploadImageBuffer(
  ...
  try {
    ...
    const commitTimestamp = new Date().toISOString().replace(/[:\-]/g, '').replace(/\.\d{3{1}}Z$/, 'Z');
