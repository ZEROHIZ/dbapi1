/**
 * @file smart-poller.ts
 * @description 即梦轮询器，支持根据任务处理进度动态调整轮询时间间隔，并在网络中断或超时情况下进行重试。
 */

import logger from "@/lib/logger.ts";
import { STATUS_CODE_MAP, POLLING_CONFIG } from "@/jimeng/consts/common.ts";
import { handlePollingTimeout, handleGenerationFailure } from "@/jimeng/lib/error-handler.ts";
