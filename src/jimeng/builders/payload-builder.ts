 ...
 * @description 即梦请求负载（payload）构造器，统一对各种分辨率、图片参数、区域参数等进行校验并构建满足上游API的请求包。
 */

import util from "@/lib/util.ts";
import { DRAFT_MIN_VERSION, DRAFT_VERSION, RESOLUTION_OPTIONS, RESOLUTION_OPTIONS_NANOBANANAPRO_4K } from "@/jimeng/consts/common.ts";
import { RegionInfo, getAssistantId } from "@/jimeng/controllers/core.ts";
...
export interface BuildCoreParamOptions {
  ...
  prompt: string;
  imageCount?: number;  // 图生图时的图片数量，用于生成 dynamic ## 前缀
