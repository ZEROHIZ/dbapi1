/**
 * @file videos.ts
 * @description 即梦视频生成控制器，处理首尾帧图片上传、全能模式(omni_reference)多素材处理与动态提示词解析、火山引擎VOD视频直传及高质量播放链接获取。
 */
...
import { SmartPoller, PollingStatus } from "@/jimeng/lib/smart-poller.ts";
import { DEFAULT_ASSISTANT_ID_CN, DEFAULT_ASSISTANT_ID_US, DEFAULT_ASSISTANT_ID_HK, DEFAULT_ASSISTANT_ID_JP, DEFAULT_ASSISTANT_ID_SG, DEFAULT_VIDEO_MODEL, DRAFT_VERSION, DRAFT_VERSION_OMNI, OMNI_BENEFIT_TYPE, OMNI_BENEFIT_TYPE_FAST, VIDEO_MODEL_MAP, VIDEO_MODEL_MAP_US, VIDEO_MODEL_MAP_ASIA } from "@/jimeng/consts/common.ts";
import { uploadImageBuffer, ImageUploadResult } from "@/jimeng/lib/image-uploader.ts";
import { uploadVideoBuffer, VideoUploadResult } from "@/jimeng/lib/video-uploader.ts";
import { extractVideoUrl, fetchHighQualityVideoUrl } from "@/jimeng/lib/image-utils.ts";
import { uploadVideoFromUrl } from "@/jimeng/lib/video-uploader.ts";
...
export async function generateVideo(
  ...
  if (isOmniMode) {
    ...
    let first_frame_image: any = undefined;
    let end_frame_image: any = undefined;
