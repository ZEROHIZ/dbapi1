/**
 * @file core.ts
 * @description 即梦核心控制器，负责通用请求封装、Cookie生成（支持sessionid与seenid兼容）、接口签名、网络重试控制、区域判定及文件/图片上传预处理等。
 */
...
import EX from "@/jimeng/consts/exceptions.ts";
import logger from "@/lib/logger.ts";
import util from "@/lib/util.ts";
import { JimengErrorHandler, JimengErrorResponse } from "@/jimeng/lib/error-handler.ts";
import { BASE_URL_DREAMINA_US, BASE_URL_DREAMINA_HK, DA_VERSION, WEB_VERSION } from "@/jimeng/consts/dreamina.ts";
...
export async function request(
  ...
  uri: string,
  refreshToken: string,
  options: AxiosRequestConfig & { noDefaultParams?: boolean; useBrowserProxy?: boolean } = {}
) {
  ...
  // 拦截使用浏览器代理的请求 (例如 Seedance 模型)
  if (options.useBrowserProxy && uri === "/mweb/v1/aigc_draft/generate") {
    logger.info(`拦截到使用浏览器代理的请求: ${uri}`);
    try {
      const { browserService } = require('../services/browser-service');
      const sessionId = util.md5(tokenWithRegion);
      const referer = headers.Referer || origin;
      
      // 去除不需要或可能导致浏览器报错的 header
      const browserHeaders = _.omit(headers, ['Cookie', 'Host', 'Content-Length']);
      
      const response = await browserService.executeGenerateRequest(
        sessionId,
        headers.Cookie, // 将 Cookie 传递给 BrowserService
        referer,
        fullUrl,
        options.data,
        browserHeaders
      ...
      // 返回的 response 需要符合 checkResult 的期望
      // response 包含 status, data
      if (response.status >= 400) {
        throw new Error(`[BrowserProxy Error] Status ${response.status}: ${JSON.stringify(response.data)}`);
      }
      return checkResult({ status: response.status, data: response.data } as any);
    } catch (err: any) {
      logger.error(`[BrowserProxy] 代理请求失败: ${err.message}`);
      throw err;
    }
  }
