/**
 * @file images.ts
 * @description 即梦图像控制器，负责文生图、图生图（混合编排）的核心请求包装、分区域模型参数转换、内容审计校验及轮询结果提取。
 */
...
import { DEFAULT_IMAGE_MODEL, DEFAULT_IMAGE_MODEL_US, IMAGE_MODEL_MAP, IMAGE_MODEL_MAP_US, IMAGE_MODEL_MAP_ASIA } from "@/jimeng/consts/common.ts";
import { uploadImageFromUrl, uploadImageBuffer } from "@/jimeng/lib/image-uploader.ts";
import { extractImageUrls } from "@/jimeng/lib/image-utils.ts";
import {
  resolveResolution,
  getBenefitCount,
  buildCoreParam,
  ...
  ResolutionResult,
} from "@/jimeng/builders/payload-builder.ts";
