...
export function sanitizeLogString(input: string): string {
  try {
    ...
    while (idx !== -1) {
      ...
      const start = idx;
      let count = 0;
      while (end < s.length && /[A-Za-z0-9+/=]/.test(s[end])) {
        end++;
        count++;
      }
    ...
    s = s.replace(BASE64_LIKE_RE, m => `[[OMITTED_BASE64 len=${m.length}]]`);
...
export function sanitizeLogValue(value: any, seen = new WeakSet<object>()): any {
  try {
    ...
    if (typeof value === "object") {
      ...
      if (typeof toJSON === "function") {
        try {
          return sanitizeLogValue(toJSON.call(value), seen);
        } catch {
          // ignore and fall back to plain object traversal
        }
      }

      const output: any = {};
      for (const [key, item] of Object.entries(value)) {
        output[key] = sanitizeLogValue(item, seen);
      }
      return output;
    }
    return value;
...
export function sanitizeLogArgs(params: any[]): any[] {
  return params.map(param => sanitizeLogValue(param));
}
...
class LogText {
  ...
  constructor(level, ...params) {
    this.level = level;
    const raw = _util.format.apply(null, sanitizeLogArgs(params));
